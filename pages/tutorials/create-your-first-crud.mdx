...
export default function PageAdminProjectUpdate() {
  ...
  const params = useParams();
  const router = useRouter();
  const repository = trpc.projects.getById.useQuery(
    {
      id: params?.id?.toString() ?? "",
    },
    {
...
To expose the page component, we need to create the route in the NextJS app router.<br />
Create a `page.tsx` file in the a new `update` folder in `src/app/admin/(authenticated)/[id]` folder.
...
export default function PageAdminProject() {
  ...
  const projectRemove = trpc.projects.removeById.useMutation({
    onSuccess: async () => {
      await trpcUtils.projects.getAll.invalidate();
    ...
    onError: () => {
      toastError({
        ...
        description: "Failed to removed the project",
      });
    },
  });
          ...
          <>
            ...
            >
              <IconButton
                ...
                isLoading={projectRemove.isLoading}
              />
            </ConfirmModal>
          </>
        }
