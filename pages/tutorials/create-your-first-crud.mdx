...
export const ProjectForm = () => {
  ...
  return (
    <Stack spacing={4}>
      <FormField control={form.control} type="text" name="name" label="Name" />
      <FormField
...
```tsx {1-2, 10-11, 16-18, 21-29, 50} filename="src/features/projects/PageAdminProjectCreate.tsx" showLineNumbers
...
You can test the form at [localhost:3000/admin/projects](http://localhost:3000/admin/projects) by using the create button we previously added, the page should looks like this.
...
<Callout type="warning">
  ...
  all projects, `PageAdminProject.tsx` will be to display a project details
</Callout>
...
export default function PageAdminProject() {
  ...
  const projectDelete = trpc.projects.removeById.useMutation({
    onSuccess: async () => {
      await trpcUtils.projects.getAll.invalidate();
      router.replace(`${ADMIN_PATH}/projects`);
    },
    onError: () => {
      toastError({
...
```tsx {13-15} filename="src/features/admin/AdminNavBar.tsx" showLineNumbers
/* ... */
