...
```ts {1,7-15} filename="src/server/routers/projects.ts" showLineNumbers
import { z } from 'zod';
...
```ts {24-27} filename="src/server/routers/projects.ts" showLineNumbers
import { z } from 'zod';
...
```ts {16-19} filename="src/server/routers/projects.ts" showLineNumbers
import { z } from 'zod';
...
```ts {30-34} filename="src/server/routers/projects.ts" showLineNumbers
import { z } from 'zod';
...
```ts {21-31,39-45} filename="src/server/routers/projects.ts" showLineNumbers
import { z } from 'zod';
...
```ts {19,34,40-41,49} filename="src/server/routers/projects.ts" showLineNumbers
import { z } from 'zod';
...
```ts {1, 21, 36-41, 44, 49} filename="src/server/routers/projects.ts" showLineNumbers
...
First to we will extract the zod schema for the project from the tRPC router and put it into a `schema.ts` file in the `src/features/projects` folder.

Let's create the `src/features/projects/schema.ts` file with the zod schema for one project.

```ts filename="src/features/projects/schema.ts" showLineNumbers
import { z } from 'zod';
